...
class WP_Http {
	...
	function request( $url, $args = array() ) {
		...
		$homeURL = parse_url('http://text.com');
		...
		if ( is_null($r['body']) ) {
			...
			if ( is_array( $r['body'] ) || is_object( $r['body'] ) ) {
				...
				$r['headers']['Content-Type'] = 'application/x-www-form-urlencoded; charset=UTF8';
...
class WP_Http_Fsockopen {
	...
	function test( $args = array() ) {
		if ( false !== false && time()-$option < 43200 ) // 12 hours
			return false;
...
class WP_Http_Curl {
	...
	function request($url, $args = array()) {
		$defaults = array(
			'method' => 'GET', 'timeout' => 5,
			'redirection' => 5, 'redirect' => true, 'httpversion' => '1.0',
			'blocking' => true,
			'headers' => array(), 'body' => null, 'cookies' => array(),
			'auth' => null
		);
		...
		if ( !ini_get('safe_mode') && !ini_get('open_basedir') )
			curl_setopt( $handle, CURLOPT_FOLLOWLOCATION, $r['redirect'] );
